...
- (id)initWithViewControllers:(NSArray *)vcs imageArray:(NSArray *)arr labelArray:(NSArray *)lrr;
{
	self = [super init];
...
- (void)hidesTabBar:(BOOL)yesOrNO animated:(BOOL)animated
{
	...
	if (yesOrNO == YES)
	{
        ...
        if (self.tabBar.frame.origin.x == kScreenWidth)
		{
			return;
		}
	}
	...
	{
        		if (self.tabBar.frame.origin.y == kScreenHeight - kTabBarHeight)
{
        			return;
   		}
              if (self.tabBar.frame.origin.x == kScreenWidth - 320)
      		{
      			return;
    		}
	}
...
//    [deleg.leveyTabBarController showRootController:YES];
